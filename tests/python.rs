...
#[test]
fn python_rejects_ast_depth_before_recursive_analysis_and_keeps_safe_depth_valid() {
    let safe_source = format!(
        "def safe():\n    return {}value{}\n",
        "(".repeat(64),
        ")".repeat(64)
    );
    let safe_output = run_with_stdin(
        &["--language", "python", "--format", "json", "-"],
        &safe_source,
    );

    assert_eq!(safe_output.status.code(), Some(0));
    let safe_report: serde_json::Value =
        serde_json::from_slice(&safe_output.stdout).expect("output should be valid JSON");
    assert_eq!(safe_report["files"][0]["status"], "ok");
    assert_eq!(safe_report["files"][0]["functions"][0]["name"], "safe");
    ...
    let unsafe_output = run_with_stdin(
        &["--language", "python", "--format", "json", "-"],
        &unsafe_source,
    );

    assert_eq!(unsafe_output.status.code(), Some(2));
    let unsafe_report: serde_json::Value =
        serde_json::from_slice(&unsafe_output.stdout).expect("output should be valid JSON");
    assert_eq!(unsafe_report["status"], "incomplete");
    assert_eq!(unsafe_report["files"][0]["status"], "parse_error");
    assert_eq!(
        unsafe_report["files"][0]["signals"],
        serde_json::Value::Null
    );
    assert_eq!(
        unsafe_report["files"][0]["functions"],
        serde_json::json!([])
    );
    assert_eq!(
        unsafe_report["files"][0]["diagnostics"][0]["message"],
