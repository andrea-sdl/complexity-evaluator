...
pub(crate) fn analyze_source(path: &str, source: &str, max_complexity: u32) -> FileResult {
    let mut tree = match parse_php_tree(source) {
        Ok(tree) => tree,
        Err(message) => return failed_file(path, message, Position { line: 1, column: 1 }),
    };
    let positions = LineIndex::new(source);
    if let Some(result) = depth_limit_failure(path, tree.root_node(), &positions) {
        return result;
    }
    tree = match reparse_reserved_class_constants(&tree, source) {
        Ok(Some(tree)) => {
            if let Some(result) = depth_limit_failure(path, tree.root_node(), &positions) {
                return result;
            }
...
fn depth_limit_failure(
    path: &str,
    root: Node<'_>,
    positions: &LineIndex<'_>,
) -> Option<FileResult> {
    if !tree_depth_exceeds_analysis_limit(root) {
        return None;
    }
...
fn tree_depth_exceeds_analysis_limit(root: Node<'_>) -> bool {
...
fn reparse_reserved_class_constants(tree: &Tree, source: &str) -> Result<Option<Tree>, String> {
    if !tree.root_node().has_error() {
        return Ok(None);
    }

    let Some(masked_source) = mask_reserved_class_constant_names(tree.root_node(), source) else {
        return Ok(None);
    };
    let reparsed_tree = parse_php_tree(&masked_source)?;
    if reparsed_tree.root_node().has_error() {
        return Ok(None);
    }

    Ok(Some(reparsed_tree))
}
