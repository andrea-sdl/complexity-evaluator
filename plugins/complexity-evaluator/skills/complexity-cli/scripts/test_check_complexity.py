...
class CheckerTests(unittest.TestCase):
    ...
    def test_missing_binary_reports_blocked(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            source = root / "source.js"
            source.write_text("function example() { return true; }\n")
            result = subprocess.run(
                [
                    sys.executable,
                    str(SCRIPT),
                    "--binary",
                    str(root / "missing-complexity"),
                    str(source),
                ],
                check=False,
                capture_output=True,
        ...
        self.assertIn("BLOCKED complexity", result.stdout)
        self.assertIn("binary is not executable", result.stdout)


class SkillContractTests(unittest.TestCase):
    def test_codex_install_docs_use_current_skill_location(self) -> None:
        documents = [
            SKILL.parents[3] / "README.md",
            SKILL.parents[2] / "README.md",
            SKILL.parent / "references" / "hooks.md",
        ]

        for document in documents:
            contents = document.read_text()
            self.assertNotIn("$HOME/.codex/skills/complexity-cli", contents)
            self.assertIn("$HOME/.agents/skills/complexity-cli", contents)

    def test_skill_stays_explicit_only_in_both_hosts(self) -> None:
        skill = SKILL.read_text()
        codex_metadata = (SKILL.parent / "agents" / "openai.yaml").read_text()
