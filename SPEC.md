...
only `tests/javascript.rs` and the skill checker's Git-conflict test with files
from a fixed repair commit. The full test suite then accepts both valid probe
outcomes and proves that its merge setup created an unmerged file. The workflow
also runs the two hook-budget tests alone with the optimized release binary and
a five-second limit. Build jobs use the unchanged tag without the test overlay
